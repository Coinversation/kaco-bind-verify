...
export default async (request: VercelRequest, response: VercelResponse) => {
  ...
  if (
    polkadotPublicKey &&
    typeof polkadotPublicKey === "string" &&
    evmAddr &&
    typeof evmAddr === "string" &&
    sign &&
    typeof sign === "string" &&
    ethers.utils.isAddress(evmAddr)
  ) {
    await cryptoWaitReady();
    result.isValidSign = isValidSignature(evmAddr, sign, polkadotPublicKey);
    if (result.isValidSign) {
      result.publicKey =
        "0x" + Buffer.from(decodeAddress(polkadotPublicKey)).toString("hex");
    }
  }
